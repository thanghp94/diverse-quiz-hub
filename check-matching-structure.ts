...
async function checkMatchingStructure() {
  try {
    // Look for matching-related tables
    const tablesQuery = `
      SELECT table_name 
      FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND table_name ILIKE '%matching%'
      ORDER BY table_name;
    `;
    
    const tables = await sourcePool.query(tablesQuery);
    console.log('Matching-related tables found:');
