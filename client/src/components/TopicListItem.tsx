...
const LocalContentThumbnail = ({ content, onClick, isGroupCard = false }: { content: any, onClick?: () => void, isGroupCard?: boolean }) => {
  ...
  return (
    <img 
      src={imageUrl} 
      alt={content.title} 
      className="w-full h-full object-contain bg-gray-800 rounded-md cursor-pointer hover:opacity-80 transition-opacity"
      onClick={onClick}
    />
  );
};
                                          ...
                                          <div className="flex flex-col">
                                                ...
                                                {groupedContent.length > 0 && (
                                                  <div className="flex flex-wrap gap-2">
                                                    {groupedContent.map((groupItem) => (
                                                      <div key={`thumb-${groupItem.id}`} className="w-20 h-16 rounded-md overflow-hidden flex-shrink-0 cursor-pointer hover:opacity-80 transition-opacity bg-gray-800">
                                                        <LocalContentThumbnail 
                                                          content={groupItem} 
                                                          onClick={() => onContentClick({
                                                            content: groupItem,
                                            ...
                                            {content.short_description && (
                                              <div className="mt-3 text-center">
                                                <p className="text-white/60 text-sm leading-relaxed">{formatDescription(content.short_description)}</p>
                                              </div>
                                            )}
                                          </div>
                                        ) : (
                                          /* Normal Content Card Layout */
                                          <div className="flex items-start justify-between gap-2">
                                            <div
                                              onClick={() => onContentClick({
                                                content,
