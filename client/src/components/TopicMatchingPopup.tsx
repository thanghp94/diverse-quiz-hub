...
import {
  ...
  MatchingActivityTrackerRef,
} from "./MatchingActivityTracker";
...
const generateTopicBasedQuestions = async (
  topicId: string,
  activities: MatchingActivityData[],
): Promise<Question[]> => {
  ...
  console.log(
    `Found ${topicContent.length} content items for topic ${topicId}`,
  );
  ...
  for (const contentItem of topicContent) {
    const image = images.find(
      (img) =>
        img.contentid === contentItem.id || img.id === contentItem.imageid,
    );
...
export const TopicMatchingPopup = ({
  isOpen,
  onClose,
  topicId,
  topicName,
}: TopicMatchingPopupProps) => {
  ...
  const handleAnswer = (answer: any, isCorrect: boolean) => {
    ...
    const totalPairs =
      currentQuestion?.pairs?.length || Object.keys(answer).length;
    let correctCount = 0;
            ...
            {isLoading || isLoadingQuestions ? (
              ...
              <div className="flex-1 flex justify-center items-center bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg">
                <p className="text-red-500">
                  Error loading matching activities.
                </p>
              </div>
            ) : questions.length === 0 ? (
              <div className="flex-1 flex justify-center items-center bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-yellow-500 text-lg">
                    No matching activities found for this topic.
                  </p>
                  <p className="text-gray-400 text-sm mt-2">
                    This topic may not have enough content with images for
                    matching activities.
                  </p>
                </div>
              </div>
